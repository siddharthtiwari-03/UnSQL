...
class UnSQL {
    ...
    static async find({ alias = null, select = [], joinSelect = [], decrypt = [], cypher = null, subQueries = [], count = [], sum = [], avg = [], least = [], join = [], where = [], whereOr = [], junction = 'AND', groupBy = null, having = null, orderBy = null, orderDirection = 'DESC', rowCount = null, offset = null }) {
        ...
        if (!this.POOL) return { success: false, error: 'Connection pool not defined! Please asign a mysql connection pool variable as a \'static POOL="your_mysql_connection_pool"\' inside model class.' }
        if (!this.TABLE_NAME) return { success: false, error: 'Database table name not mapped! Please asign a mysql table name variable as a \'static TABLE_NAME="db_table_name"\' inside model class.' }
    ...
    static save({ alias = null, data, updateObj = null, where = null, whereOr = null, junction = 'AND' }) {
        return new Promise((resolve, reject) => {
            if (!this.POOL) return resolve({ success: false, error: 'Connection pool not defined! Please asign a mysql connection pool variable as a \'static POOL="your_mysql_connection_pool"\' inside model class.' })
            if (!this.TABLE_NAME) return resolve({ success: false, error: 'Database table name not mapped! Please asign a mysql table name variable as a \'static TABLE_NAME="db_table_name"\' inside model class.' })
            let sql = ''
            ...
            if (where != null || whereOr != null) {
                ...
                if (where && whereOr && junction) sql += ` ${junction || 'AND'} `
