...
Beyond this version (i.e. v1.6+) we are completely dropping the support for older **callback** based approach and also the dependency on **mysql** library in favour of newer **promise** based and **mysql2**. You can easily swap in the new version by replacing the older **mysql** import with **mysql2/promise** and you would be good to go. Also, we will be dropping the support for functional based methods, retainng the class based approach only.
